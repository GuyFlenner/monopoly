...
from kesef_engine.rules import auction, endgame, turns
...
MORTGAGE_TRANSFER_RATE = 10
"""Percent of the mortgage value the receiver owes the bank at transfer (owner decision 2)."""


def mortgage_transfer_fee(tile: Tile) -> int:
    """The 10% a mortgaged property costs its new owner, rounded up (G-13).

    Rounds the same way :func:`~kesef_engine.legality.unmortgage_cost` rounds its interest,
    so the two halves of the official mortgage rule cannot disagree about a half-shekel.
    Public because MON-204 charges the identical fee on the identical event — a mortgaged
    deed changing hands — and one arithmetic is the point.
    """
    value = tile.mortgage or 0
    return -(-value // MORTGAGE_TRANSFER_RATE)
